...
async function remove(path) {
	return await new Promise((resolve, reject) => fs.remove(path, (error) => (error) ? reject(error) : resolve()))
}
...
async function loadJsonData(z) {
	if(z > fileNames.length - 1) { 
		...
		await saveToDb(data, 0, collectionName, async() => await loadJsonData(z + 1));
	}
}
...
async function saveToDb(fileData, x, collectionName) {
	...
	const result = await col.update({ "_id": collection._id }, collection, { upsert: true });
