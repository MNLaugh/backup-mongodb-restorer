...
async function restore (dbaseUri, pathToZipFile, useObjectID = false) {
...
async function saveToDb(fileData, x, collectionName) {
	...
	if(isObjectID) collection._id = ObjectID.createFromHexString(collection._id);
	...
	let _id = collection._id //(collection._id.length !== 24) ? collection._id : ObjectID(collection._id);
	try {
		const result = await col.replaceOne({ "_id": _id }, collection, { upsert: true });
	} catch(e) {
		console.error("Error replaceOne with id: " + _id, e);
		throw e;
	}
